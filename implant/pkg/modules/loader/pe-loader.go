...
func LoadPe(peBytes []byte) {
	...
	for {
		...
		exitThreadAddr := wc.GetFunctionAddress(k32, wc.GetHash("ExitThread"))
		ntdll := wc.GetModuleBase(wc.GetHash("ntdll.dll"))
		rtlExitUserThreadAddr := wc.GetFunctionAddress(ntdll, wc.GetHash("RtlExitUserThread"))
		...
		for {
			...
			if funcName == "ExitProcess" || funcName == "exit" || funcName == "_exit" || funcName == "_Exit" {
				if exitThreadAddr != 0 {
					funcAddr = exitThreadAddr
				}
			} else if funcName == "RtlExitUserProcess" || funcName == "NtTerminateProcess" || funcName == "ZwTerminateProcess" {
				if rtlExitUserThreadAddr != 0 {
					funcAddr = rtlExitUserThreadAddr
				}
			}
	...
	if tHandle != 0 {
		wc.Call("kernel32.dll", "CloseHandle", tHandle)
	}
}
