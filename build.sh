...
sed "s/HOST_PLACEHOLDER/${ZIG_HOST_ARRAY}0/g; s/PORT_PLACEHOLDER/$STAGER_PORT/g; s/USE_TLS_PLACEHOLDER/$USE_TLS/g" stager.zig > stager_build.zig
zig build --prefix ../build
mv ../build/bin/stager.exe ../build/stager_zig.exe
rm -rf ../build/bin
rm -f stager_build.zig
echo "    [Nim] building stager_nim.exe..."
sed "s/HOST_PLACEHOLDER/$STAGER_HOST/g; s/PORT_PLACEHOLDER/$STAGER_PORT/g; s/USE_TLS_PLACEHOLDER/$USE_TLS/g" stager.nim > stager_build.nim
...
RUST_HOST_ARRAY+="0]"
sed "s/HOST_PLACEHOLDER/$RUST_HOST_ARRAY/g; s/PORT_PLACEHOLDER/$STAGER_PORT/g; s/USE_TLS_PLACEHOLDER/$USE_TLS/g" stager.rs > stager_build.rs
