...
func SaveImplant(meta *proto.ImplantMeta, ip string) error {
	dbLock.Lock()
	defer dbLock.Unlock()
	...
	_, err := database.Exec(`
		...
		(id, hostname, username, domain, os, arch, pid, process, elevated, first_seen, last_seen, sleep, jitter, alive, last_ip)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?)`,
		meta.ID, meta.Hostname, meta.Username, meta.Domain, meta.OS, meta.Arch,
		meta.PID, meta.Process, elevated, firstSeen, lastSeen, meta.Sleep, meta.Jitter, ip)
	return err
}

func UpdateImplantLastSeen(id string, ip string) error {
	dbLock.Lock()
	defer dbLock.Unlock()

	lastSeen := time.Now().UTC().Format(time.RFC3339)
	_, err := database.Exec(`UPDATE implants SET last_seen = ?, alive = 1, last_ip = ? WHERE id = ?`, lastSeen, ip, id)
	return err
}
...
func GetImplant(id string) (*Implant, error) {
	...
	err := database.QueryRow(`SELECT * FROM implants WHERE id = ?`, id).Scan(
		...
		&i.PID, &i.Process, &elevated, &firstSeen, &lastSeen, &i.Sleep, &i.Jitter, &alive, &lastIP)
	if err != nil {
		return nil, err
	}
	if lastIP.Valid {
		i.LastIP = lastIP.String
	}
...
func GetAllImplants() ([]*Implant, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&i.ID, &i.Hostname, &i.Username, &i.Domain, &i.OS, &i.Arch,
			&i.PID, &i.Process, &elevated, &firstSeen, &lastSeen, &i.Sleep, &i.Jitter, &alive, &lastIP); err != nil {
			continue
		}
		if lastIP.Valid {
			i.LastIP = lastIP.String
...
func GetPendingScreenshotTaskByIP(ip string) (*Task, error) {
	dbLock.RLock()
	defer dbLock.RUnlock()
	...
	err := database.QueryRow(`
		SELECT t.id, t.implant_id, t.type, t.args, t.data, t.status, t.output, t.error, t.created, t.completed
		FROM tasks t
		JOIN implants i ON t.implant_id = i.id
		WHERE i.last_ip = ? AND t.type = 'bof' AND t.status IN ('pending', 'running', 'complete')
		ORDER BY t.created DESC
		LIMIT 1`,
		ip).Scan(&t.ID, &t.ImplantID, &t.Type, &t.Args, &data, &t.Status, &output, &errorStr, &created, &completed)
...
func UpdateTaskArgs(taskID string, args []string) error {
	dbLock.Lock()
	defer dbLock.Unlock()
	...
	_, err = database.Exec(`UPDATE tasks SET args = ? WHERE id = ? AND status = 'pending'`, string(argsJSON), taskID)
	return err
}
